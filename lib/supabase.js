...
export function describeKey() {
  ...
  // The commonest paste error of the lot, and the least obvious from a 401:
  // the same value typed into both fields.
  if (/^https?:\/\//i.test(KEY)) {
    return KEY === URL_BASE || KEY.replace(/\/+$/, '') === URL_BASE
      ? `a URL, and the SAME one as SUPABASE_URL (${KEY.length} chars) — the project URL has been pasted into both fields`
      : `a URL, not a key (${KEY.length} chars) — something other than the secret key has been pasted here`;
  }
...
export function diagnose() {
  ...
  if (/^https?:\/\//i.test(KEY)) {
    lines.push(
      '',
      '  SUPABASE_SERVICE_KEY needs the key, not an address. Copy it from',
      '  Project Settings -> API Keys -> Secret keys. It begins "sb_secret_".',
    );
