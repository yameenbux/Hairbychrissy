...
/**
 * Never hand out a booking reference that already exists.
 *
 * The counter lives in the settings row, and the bookings live in their own
 * table, so the two can drift apart: a settings write that did not land, or
 * the moment during a deploy when the outgoing and incoming processes are both
 * alive and each holds its own copy of the counter. It happened in production
 * on the first day — HBC-1001 was issued twice, and the second client was
 * turned away with "we could not save your booking" while the database sat
 * there perfectly healthy.
 *
 * The bookings themselves are the authority on what has been issued, so the
 * counter is trued up against them at boot. Self-healing rather than
 * preventative: whatever caused the drift, the next reference is past it.
 */
function reconcileCounter() {
  const highest = (db.bookings || []).reduce((max, b) => {
    const n = Number(String(b.ref || '').replace(/\D+/g, ''));
    return Number.isFinite(n) && n > max ? n : max;
  }, 0);

  if (highest > (db.counter || 0)) {
    console.warn(
      `[store] booking counter was ${db.counter}, but ${highest} has already been issued — moving it forward`,
    );
    db.counter = highest;
  }
}
