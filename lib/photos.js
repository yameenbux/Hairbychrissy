 ...
 *   - Files land in data/uploads, OUTSIDE public/, so no web server will ever
 *     serve them by accident. They come back out only through an
...
export function readPhoto(bookingId, file) {
  // The stored name is one we generated, but it arrives back through a URL,
  // so it is checked again rather than trusted a second time.
  if (!/^[0-9]+\.(jpg|png|gif|webp|heic)$/.test(file)) return null;
  try {
...
export function removePhotos(bookingId) {
  try {
