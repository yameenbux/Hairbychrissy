...
/**
 * Read a Checkout Session back from Stripe to confirm the deposit really landed.
 *
 * The client is redirected to /confirmed after paying, and that page asks the
 * server to verify — so the booking is confirmed (and Chrissy notified) from
 * Stripe's own answer, never from anything the browser claims.
 *
 * A Stripe webhook is the belt-and-braces version of this, since it fires even
 * if the client closes the tab before the redirect. See README.
 */
