...
export const brand = {
  ...
  /*
   * From the sample site she built herself, and hers to keep or drop.
   *
   * `area` NEEDS HER NOD. This site has deliberately said only "London", with
   * the exact address sent on booking, because she works alone from a private
   * studio. A borough is not an address and she published it herself, twice,
   * on her own sample — but widening her public footprint is her decision to
   * confirm, not mine to infer, so nothing renders it yet.
   */
  area: 'Bermondsey',
  since: 2018,
  welcome:
