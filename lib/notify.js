...
async function resendSend(payload) {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    throw new Error(explain(res.status, body, payload));
  }
}

/**
 * Turn Resend's refusal into an instruction.
 *
 * The one that matters is the unverified-sender trap, and it is nasty because
 * it is INVISIBLE until a real client books. The shared onboarding@resend.dev
 * sender only delivers to the address the Resend account itself is registered
 * under. So the stylist's own alert arrives — that address is the account —
 * and every client confirmation is refused, because a client is by definition
 * somebody else. Nothing about the symptom points at the sender, so the raw
 * API body gets read as "email is broken" rather than "verify a domain".
 */
function explain(status, body, payload) {
  const to = [].concat(payload?.to || []).join(', ');
  if (status === 403 && /verify a domain|your own email address/i.test(body)) {
    return `Resend refused to deliver to ${to}.\n\n`
      + `  The sender is ${payload?.from || 'unset'}. Resend's shared sender only delivers to the\n`
      + `  address your Resend account is registered under, so this works for your own\n`
      + `  test alerts and fails for every real client.\n\n`
      + `  Fix: verify a domain at resend.com/domains, then set NOTIFY_EMAIL_FROM to an\n`
      + `  address at that domain. Until then no client will receive a confirmation.`;
  }
  if (status === 401 || status === 403) {
    return `Resend rejected the request (${status}) — check RESEND_API_KEY. ${body.slice(0, 160)}`;
  }
...
async function emailChannel(note) {
  ...
  /*
   * Name the one that is missing. "RESEND_API_KEY / NOTIFY_EMAIL_TO not set"
   * is no help when the client's copy is arriving fine — that already proves
   * the key works, so the answer is always the address, and the message
   * should say so rather than make somebody guess between two.
   */
  if (!key || !to) {
    return { skipped: !key && !to
      ? 'Neither RESEND_API_KEY nor NOTIFY_EMAIL_TO is set.'
      : !key
        ? 'RESEND_API_KEY is not set.'
        : 'NOTIFY_EMAIL_TO is not set — nobody is listed to receive her booking alerts.' };
  }
  ...
  /*
   * Point replies at the CLIENT who just booked, so answering a booking alert
   * is one tap and reaches the person it is about.
   *
   * This also removes the need for a mailbox at the sending domain. The from
   * address only has to be able to SEND — nobody ever has to receive at it,
   * because every reply on either email is steered to a real inbox instead.
   */
  await resendSend({
    from: process.env.NOTIFY_EMAIL_FROM || 'Bookings <onboarding@resend.dev>',
    ...(note.replyTo ? { reply_to: [note.replyTo] } : {}),
    to: recipients,
    subject: note.title,
    text: note.body,
    html: emailHtml(note),
 ...
 */
export async function notify({ kind, title, body, booking = null, replyTo = null }) {
  /*
   * replyTo rides on the note rather than inside `booking`, because `booking`
   * is the shape written to the stored notification log and this is only ever
   * needed at the moment of sending.
   */
  const note = { kind, title, body, booking, replyTo, at: new Date().toISOString() };
...
export function bookingMessage(booking, service) {
  ...
  return {
    ...
    // So hitting reply on the alert answers the client directly.
    replyTo: booking.client.email || null,
  };
}
